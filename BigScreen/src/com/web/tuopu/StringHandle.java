...
public class StringHandle {
	...
	//返回日期的时间
	public int dateToWeek(String pTime,String informat) {  
	...
	//将字符串容器转化为数字容器
	public List<Integer> StringListToIntegerList(List<String> ls)
	{
		List<Integer> result=new ArrayList<Integer>();
		int g_num=ls.size();
		for(int i=0;i<g_num;i++)
		{
			result.add(Integer.parseInt(ls.get(i)));
			
		}
		return result;
	}
	//将时间字符串转化为另一格式 yyyy-MM-dd HH:mm:ss
	//System.out.println(sh.DateToAnotherDateFormat("2020-5-8", "yyyy-MM-dd", "yyyy/MM"));
	public String DateToAnotherDateFormat(String time,String informat,String outformat)
	{
		
		Date date = new Date();
        try {
            date = new SimpleDateFormat(informat).parse(time);
            
        } catch (ParseException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        String now = new SimpleDateFormat(outformat).format(date);
        return now;
	}
